...
function Contact() {
  ...
  const sendForm = async (e: FormEvent) => {
    ...
    setIsLoading(true)
    try {
      await emailjs.send('service_6a99t22', 'contact_form', contactMe, {
        publicKey: 'dAirB3vUzJJ_W6lRi',
      })
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
        Send a message to make any enquiries.
      </p>
      <div className="mt-6 flex flex-col gap-5">
        <input
          ...
          aria-label="Name"
          name="user_name"
          value={contactMe?.user_name}
          onChange={(e) =>
            setContactMe({ ...contactMe, user_name: e.target.value })
          }
          required
          className="min-w-0 flex-auto appearance-none rounded-md border border-zinc-900/10 bg-white px-3 py-[calc(--spacing(2)-1px)] shadow-md shadow-zinc-800/5 placeholder:text-zinc-400 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 focus:outline-hidden sm:text-sm dark:border-zinc-700 dark:bg-zinc-700/15 dark:text-zinc-200 dark:placeholder:text-zinc-500 dark:focus:border-teal-400 dark:focus:ring-teal-400/10"
        />
        <input
          type="email"
          ...
          name="user_email"
          value={contactMe?.user_email}
          onChange={(e) =>
            setContactMe({ ...contactMe, user_email: e.target.value })
          }
          required
          className="min-w-0 flex-auto appearance-none rounded-md border border-zinc-900/10 bg-white px-3 py-[calc(--spacing(2)-1px)] shadow-md shadow-zinc-800/5 placeholder:text-zinc-400 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 focus:outline-hidden sm:text-sm dark:border-zinc-700 dark:bg-zinc-700/15 dark:text-zinc-200 dark:placeholder:text-zinc-500 dark:focus:border-teal-400 dark:focus:ring-teal-400/10"
        />
        <textarea
          ...
          onChange={(e) =>
            setContactMe({ ...contactMe, message: e.target.value })
          }
          placeholder="Type your message here"
          className="min-w-0 flex-auto appearance-none rounded-md border border-zinc-900/10 bg-white px-3 py-[calc(--spacing(2)-1px)] shadow-md shadow-zinc-800/5 placeholder:text-zinc-400 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 focus:outline-hidden sm:text-sm dark:border-zinc-700 dark:bg-zinc-700/15 dark:text-zinc-200 dark:placeholder:text-zinc-500 dark:focus:border-teal-400 dark:focus:ring-teal-400/10"
          required
        />
        <Button type="submit" className="md:w-1/4" disabled={isLoading}>
...
function Resume() {
  ...
  return (
    <div className="rounded-2xl border border-zinc-100 p-6 dark:border-zinc-700/40">
      ...
      <Button
        href="https://flowcv.com/resume/in85qwlwwg"
        target="_blank"
        variant="secondary"
  ...
  return (
    <div className="mt-16 sm:mt-20">
      <div className="-my-4 flex justify-center gap-5 overflow-hidden py-4 sm:gap-8">
        {[image4, image1, image5, image3, image2].map((image, imageIndex) => (
          <div
            key={image.src}
            className={clsx(
              'relative aspect-9/10 w-44 flex-none overflow-hidden rounded-xl bg-zinc-100 shadow-sm drop-shadow-sm sm:w-72 sm:rounded-2xl dark:bg-zinc-800',
      ...
      <Container className="mt-9">
        <div className="max-w-3xl">
          ...
          <div className="mt-6 flex gap-6">
            <SocialLink
              href="https://twitter.com/Grace_ffiong"
